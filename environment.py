...
class cmake:
    ...
    if isWindows():
        source = [
            [wget, "http://www.cmake.org/files/v2.6/cmake-2.6.2-win32-x86.exe", downloadPath]
        ]
        buildCmds = [
...
class scons:
    ...
    if isLinux() or isMac():
        buildCmds = [
            ...
            [0, 'sed --in-place "s/-bundle//" applelink.py', os.path.join (os.getcwd(), base, 'engine', 'SCons', 'Tool')],
...
class ogreal:
    ...
    if isWindows():
        ...
        CCFLAGS = ' -DWIN32 -DNDEBUG -D_LIB -D_WIN32 -D_WINDOWS '
        libs = [
            boost.lib,
            'OgreMain',
            'ogg_static',
...
class ogrevideoffmpeg:
    ...
    if os.name == 'nt':
        libs = [
            boost.lib, 'OgreMain', 'libavformat', 'libavcodec', 'libavutil',
...
class ogrebulletd:  #
    ...
    cflags = ""
    parent = "ogre/physics"
    libs = [boost.lib, 'OgreMain', 'LibBulletCollision', 'LibBulletDynamics', 'LibBulletSoftBody', 'LibBulletMultiThreaded', 'LibGIMPACT']
    if isWindows():
        libs.append('libbulletMath')
    else:
        libs.append('LibLinearMath')
