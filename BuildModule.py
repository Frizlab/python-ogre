...
def getClassList ():
    ...
    for c in dir(environment):
        ...
        if isinstance(var, environment.MetaClass ) : #(type, types.ClassType)):    ## OK so we know it's a class
...
####    c = StringIO()
    ...
    os.chmod(os.path.join(debiandir, "rules"), 0x755)
...
if __name__ == '__main__':
    ...
    if options.gencodeall or options.compilecodeall:
        ...
        for moduleName  in args:
            if not moduleName in classList:
