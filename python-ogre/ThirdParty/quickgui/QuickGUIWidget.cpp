...
namespace QuickGUI
{
	...
	void WidgetDesc::serialize(SerialBase* b)
	{
		...
		b->IO("Detectable",&widget_detectable);
	...
	void Widget::_initialize(WidgetDesc* d)
	{
		...
		setDetectable(d->widget_detectable);
	...
	Widget* Widget::findWidgetAtPoint(const Point& p, bool ignoreDisabled)
	{
		...
		if(!mWidgetDesc->widget_detectable)
			return NULL;
	...
	bool Widget::getDetectable()
	{
		return mWidgetDesc->widget_detectable;
	}
	...
	void Widget::setDetectable(bool detectable)
	{
		mWidgetDesc->widget_detectable = detectable;
	}
