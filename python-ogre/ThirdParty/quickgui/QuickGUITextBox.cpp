...
namespace QuickGUI
{
	TextBox::TextBox(const Ogre::String& name, const Ogre::Vector4& dimensions, GuiMetricsMode positionMode, GuiMetricsMode sizeMode, const Ogre::String& material, Ogre::OverlayContainer* overlayContainer, Widget* ParentWidget) :
		...
		mInputMode(0)
	{
		...
		mTextWidget = new Text(mInstanceName+".Text",textDimensions,QGUI_GMM_RELATIVE,QGUI_GMM_RELATIVE,mChildrenContainer,this);
	...
	void TextBox::alignText(HorizontalAlignment ha, VerticalAlignment va)
	{
		...
		relativeLabelPos = mTextWidget->getPosition();
	...
	void TextBox::maskUserInput(const Ogre::UTFString::unicode_char& symbol)
	{
		...
		if( (mText != "") && !mMaskUserInput ) setText(mText);
