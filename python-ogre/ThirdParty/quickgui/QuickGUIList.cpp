...
namespace QuickGUI
{
	List::List(const Ogre::String& name, const Ogre::Vector3& dimensions, const Ogre::String& material, const Ogre::String& highlightMaterial,Ogre::OverlayContainer* overlayContainer, Widget* ParentWidget) :
	...
	void List::_notifyZOrder(Ogre::ushort windowZOrder, Ogre::ushort offsetZOrder)
	{
		Label::_notifyZOrder(windowZOrder,offsetZOrder);
	}

	void List::_notifyZOrder(Ogre::ushort windowZOrder)
	{
		std::vector<ListItem*>::iterator it1;
		for( it1 = mItems.begin(); it1 != mItems.end(); ++it1 )
	...
	ListItem* List::addListItem(const Ogre::UTFString& text)
	{
		...
		newListItem->_notifyZOrder(mWindowZOrder,1);
