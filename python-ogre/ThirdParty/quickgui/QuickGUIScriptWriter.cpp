...
namespace QuickGUI
{
	...
	void ScriptWriter::begin(const Ogre::String& fileName)
	{
		...
		// Get existing Definitions that may be in the file. (they will be re-written back to file, but some might be overwritten)
		mDefinitions = ScriptReader::getSingletonPtr()->parseFile(fileName);
	}
