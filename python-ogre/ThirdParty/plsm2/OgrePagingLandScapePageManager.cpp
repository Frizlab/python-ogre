...
namespace Ogre
{
	...
	void PagingLandScapePageManager::makePageLoadedNow(PagingLandScapePage * p)
	{
		...
		if (!p->isLoaded())
		{
			...
			p->load();
			PLSM2_ASSERT ( std::find(mLoadedPages.begin(), mLoadedPages.end(), p) == mLoadedPages.end());
	...
	void PagingLandScapePageManager::processUnloadQueues()
	{
		...
		{
			PLSM2_ASSERT (!(*itq)->isLoaded() && !(*itq)->isPreLoaded() && !(*itq)->isTextureLoaded());
		    PLSM2_ASSERT (!(*itq)->mIsLoading && !(*itq)->mIsTextureLoading);
		...
		{
			PLSM2_ASSERT (!(*itq)->isLoaded() && (*itq)->isPreLoaded() && !(*itq)->isTextureLoaded());
		    PLSM2_ASSERT (!(*itq)->mIsLoading && (*itq)->mIsTextureLoading && !(*itq)->mIsPreLoading);
			if ((*itq)->unloadUntouched ())
			{
				p = *itq;
				// remove from queue
		...
		if (mNextQueueFrameCount-- < 0)
		{
			...
			while (camIt.hasMoreElements())
			{
				...
				if (!mPageLoadQueue.empty ())
				{
					...
					PLSM2_ASSERT (p && !p->isLoaded ());
					PLSM2_ASSERT (!p->mIsTextureLoading && !p->mIsPreLoading);
					p->load ();
				...
				else if (!mPageTextureloadQueue.empty ())
				{
					...
					PLSM2_ASSERT (p && !p->isTextureLoaded());
					PLSM2_ASSERT (!p->mIsLoading && !p->mIsPreLoading);
					p->loadTexture ();
				...
				else if (!mPagePreloadQueue.empty ())
				{
					...
					PLSM2_ASSERT (p && !p->isPreLoaded());
					PLSM2_ASSERT (!p->mIsLoading && !p->mIsTextureLoading);
					p->preload ();
    ...
    void PagingLandScapePageManager::removeFromQueues(PagingLandScapePage* p)
    {
		...
		PLSM2_ASSERT (!p->mIsLoading && !p->mIsTextureLoading && !p->mIsPreLoading);
    }
    //-----------------------------------------------------------------------
