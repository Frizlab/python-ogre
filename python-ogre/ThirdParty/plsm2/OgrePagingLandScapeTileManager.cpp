...
{
	PLSM2_ASSERT (mTiles.size() == mNumTiles);
...
{
	PLSM2_ASSERT (mTiles.size() == (unsigned int) mQueue.getSize() && 
...
void PagingLandScapeTileManager::load()
{
	...
	PLSM2_ASSERT (mOptions->num_tiles <= mTiles.size());
...
{
	PLSM2_ASSERT (!tile->isLoaded () && tile->getRenderable () == 0);
	mQueue.push (tile);
}
//-----------------------------------------------------------------------
