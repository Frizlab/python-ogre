...
namespace SimpleGUI
{
	NStateButton::NStateButton(const Ogre::String& name, const Ogre::Vector4& dimensions, bool show, Ogre::OverlayContainer* overlayContainer, Widget* ParentWidget) :
		Button(name,dimensions,"",show,overlayContainer,ParentWidget)
	{
		mWidgetType = Widget::SGUI_NSTATEBUTTON;
	}
	...
	void NStateButton::addState(const Ogre::String& name, const Ogre::String& material, Ogre::String text)
	{
	...
	bool NStateButton::onMouseButtonUp(MouseEventArgs& e)
	{
		...
		if( e.button == SimpleGUI::MB_Left ) toggleNextState();
		else if( e.button == SimpleGUI::MB_Right ) togglePreviousState();
	...
	void NStateButton::setCurrentState(State* s)
	{
		...
		mCurrentState = s;
		mMaterial = mCurrentState->getMaterial();
