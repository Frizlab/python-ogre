...
*/
	...
	std::stringstream ss;
	ss << "PhysX UserAllocator destructor debug statistics" << std::endl;

	ss << "- Alloc (" << mTotalNbAllocs << ") Realloc (" << mNbReallocs << "), Remaining (" << mNbAllocs << ")" << std::endl; 
	ss << "- High water mark (" << (mHighWaterMark/1024) << ")" << std::endl;

	if (mNbAllocatedBytes)
		ss << std::endl << "Memory leak detected (" << mNbAllocatedBytes << " bytes). Dump will follow." << std::endl << std::endl;
	...
	if(mMemBlockList && mMemBlockUsed) {
		...
		for(NxU32 i=0, j=0; i<used; i++, j++) {
			...
			ss << "- " << std::hex << "0x" << cur+6 << std::dec << ", " << cur[1] << " bytes (" << (const char*)cur[5] << "), allocated in " << (const char*)cur[5] << "(" << cur[3] << ")" << std::endl;
			NbLeaks++;
			//	Free(cur+4);
		}

		ss << std::endl << NbLeaks << " leaks in total." << std::endl;
		//printf("\n  Dump complete (%d leaks)\n\n", NbLeaks);
	}
	else {
		ss << std::endl << "No leaks allocated found." << std::endl;
	}
