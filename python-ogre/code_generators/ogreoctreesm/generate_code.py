...
## STARTER TEMPLATE..
## replace ogreoctreesm with lowercase project name
## set MAIN_NAMESPACE
...
def ManualInclude ( mb ):
    ...
    if MAIN_NAMESPACE:
        ...
        main_ns = global_ns    
    inc = ['TerrainSceneManager','OctreeSceneManager'] 
    for c in inc:
...
#           Warnings 1020 - 1031 are all about why Py++ generates wrapper for class X
    ...
    mb = module_builder.module_builder_t( [ xml_cached_fc ]
                                          ...
                                          , include_paths=environment.ogreoctreesm.include_dirs
                                          , define_symbols=defined_symbols
                                          , indexing_suite_version=2
                                          , cflags=environment.ogreoctreesm.cflags
                                           )
    ...
    common_utils.addDetailVersion ( mb, environment, environment.ogreoctreesm )
    ...
    mb.build_code_creator (module_name='_ogreoctreesm_' , doc_extractor= extractor )
    
    for inc in environment.ogreoctreesm.include_dirs:
        mb.code_creator.user_defined_directories.append(inc )
    mb.code_creator.user_defined_directories.append( environment.ogreoctreesm.generated_dir )
    mb.code_creator.replace_included_headers( customization_data.header_files( environment.ogreoctreesm.version ) )

    huge_classes = map( mb.class_, customization_data.huge_classes( environment.ogreoctreesm.version ) )

    mb.split_module(environment.ogreoctreesm.generated_dir, huge_classes, use_files_sum_repository=False)

    ## now we need to ensure a series of headers and additional source files are
    ## copied to the generated directory..
    
#     common_utils.copyTree ( sourcePath = environment.Config.PATH_INCLUDE_ogreoctreesm, 
#                             destPath = environment.ogreoctreesm.generated_dir, 
#                             recursive=False )
    if environment.ogre.version.startswith("1.7"):
        ## have a code generation issue that needs resolving...
        filesToFix=['TerrainSceneManager.pypp.cpp', 'stdVectorStringString.pypp.cpp']
        for filename in filesToFix:
            fname = os.path.join( environment.ogreoctreesm.generated_dir, filename)
            try:
                f = open(fname, 'r')
                buf = f.read()
                f.close()
