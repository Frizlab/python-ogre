...
## STARTER TEMPLATE..
## replace ogrepaging with lowercase project name
## set MAIN_NAMESPACE
...
#           Warnings 1020 - 1031 are all about why Py++ generates wrapper for class X
    ...
    mb = module_builder.module_builder_t( [ xml_cached_fc ]
                                          ...
                                          , include_paths=environment.ogrepaging.include_dirs
                                          , define_symbols=defined_symbols
                                          , indexing_suite_version=2
                                          , cflags=environment.ogrepaging.cflags
                                           )
    ...
    common_utils.addDetailVersion ( mb, environment, environment.ogrepaging )
    ...
    mb.build_code_creator (module_name='_ogrepaging_' , doc_extractor= extractor )
    
    for inc in environment.ogrepaging.include_dirs:
        mb.code_creator.user_defined_directories.append(inc )
    mb.code_creator.user_defined_directories.append( environment.ogrepaging.generated_dir )
    mb.code_creator.replace_included_headers( customization_data.header_files( environment.ogrepaging.version ) )

    huge_classes = map( mb.class_, customization_data.huge_classes( environment.ogrepaging.version ) )

    mb.split_module(environment.ogrepaging.generated_dir, huge_classes, use_files_sum_repository=False)

    ## now we need to ensure a series of headers and additional source files are
    ## copied to the generated directory..
    
#     common_utils.copyTree ( sourcePath = environment.Config.PATH_INCLUDE_ogrepaging, 
#                             destPath = environment.ogrepaging.generated_dir, 
#                             recursive=False )
