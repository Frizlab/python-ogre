...
#
    ...
    def createGUI(self):
        self.GUIManager.createMouseCursor(ogre.Vector2(0.05,0.05),"sgui.pointer")
        w = self.GUIManager.createWindow(ogre.Vector4(0,0,0.3,0.2),"sgui.window")
        w.setText("Sample Text!")
        b = w.createButton(ogre.Vector4(0.25,0.3,0.5,0.4),"sgui.button")
        b.setText("Exit!")
        self.cb1 = CallBack()
        self.cb1.info="MOUSE_BUTTON_UP"
    ...
    def my_eventHandler(self, args):
        ...
        print "\n\n", args,"\n"
        print dir(args),"\n"
        if(mea.button == gui.MouseButtonID.MB_Left):
...
class GuiFrameListener ( sf.FrameListener, ois.MouseListener, ois.KeyListener ):
    ...
    def  keyPressed( self, arg ):
        if( arg.key == ois.KC_ESCAPE ):
            self.ShutdownRequested = True
        gui.GUIManager.getSingleton().injectKeyDown( arg.key )
        gui.GUIManager.getSingleton().injectChar( arg.text )
        return True

    ##----------------------------------------------------------------##
    def keyReleased( self, arg ):
        gui.GUIManager.getSingleton().injectKeyUp(arg.key)
        return True
