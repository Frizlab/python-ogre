...
VolumeRenderable.VolumeRenderable(size_t nSlices, float size,  String &texture):
	mSlices(nSlices),
	mSize(size),
	mTexture(texture)

	mRadius = sqrtf(size*size+size*size+size*size)/2.0 
...
void VolumeRenderable._notifyCurrentCamera( Camera* cam )
	...
	rotMat = tempMat 
	rotMat.setTrans(Vector3(0.5, 0.5f, 0.5f)) 
	mUnit->setTextureTransform(rotMat) 



## Ogre.Quaternion& VolumeRenderable::getWorldOrientation(void) const

##	return Ogre.Quaternion::IDENTITY 


void VolumeRenderable.getWorldTransforms( Matrix4* xform ) 

	Matrix4 destMatrix(Matrix4.IDENTITY)  ## self initialisation is needed
...
void VolumeRenderable.initialise()
	...
	for(x=0  x<mSlices; x++) 
		...
		for(size_t y=0  y<4; y++)
			...
			float zcoord = -((float)x/(float)(mSlices-1)  - 0.5) 
			## 1.0 .. a/(a+1)
			## coordinate
			vertices[x*4*elemsize+y*elemsize+0] = xcoord*(mSize/2.0) 
			vertices[x*4*elemsize+y*elemsize+1] = ycoord*(mSize/2.0) 
			vertices[x*4*elemsize+y*elemsize+2] = zcoord*(mSize/2.0) 
			## normal
			vertices[x*4*elemsize+y*elemsize+3] = 0.0 
			vertices[x*4*elemsize+y*elemsize+4] = 0.0 
			vertices[x*4*elemsize+y*elemsize+5] = 1.0 
			## tex
			vertices[x*4*elemsize+y*elemsize+6] = xcoord*sqrtf(3.0) 
			vertices[x*4*elemsize+y*elemsize+7] = ycoord*sqrtf(3.0) 
			vertices[x*4*elemsize+y*elemsize+8] = zcoord*sqrtf(3.0) 
...
Ogre.Real VolumeRenderable::getBoundingRadius() 

	return mRadius 

Ogre.Real VolumeRenderable::getSquaredViewDepth( Ogre::Camera* cam) const
