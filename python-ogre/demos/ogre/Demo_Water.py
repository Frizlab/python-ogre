...
self.circles_MATERIAL ="Examples/Water/self.circles"
...
def prepareCircleMaterial():
    ...
    ##  
    imgstream = ogre.MemoryDataStream(ctypes.addressof(bmap),  (256 * 256 * 4 ) )  
    
    ogre.TextureManager.getSingleton().loadRawData(self.circles_MATERIAL,
        ogre.ResourceGroupManager.DEFAULT_RESOURCE_GROUP_NAME,
        imgstream, 256, 256  , ogre.PixelFormat.PF_A8R8G8B8 ) 
                    
    material = ogre.MaterialManager.getSingleton().create( self.circles_MATERIAL,
        ogre.ResourceGroupManager.DEFAULT_RESOURCE_GROUP_NAME) 
    texLayer = material.getTechnique(0).getPass(0).createTextureUnitState( self.circles_MATERIAL ) 
...
class WaterListener(sf.FrameListener):
    ...
    def processself.circles(self,timeSinceLastFrame):
...
# # #         while (found)  
    ...
    def processParticles(self):
        global PLANE_SIZE, COMPLEXITY, self.RAIN_HEIGHT_RANDOM, self.RAIN_HEIGHT_CONSTANT
...
## Just override the mandatory create scene method
    def _createScene(void)
        ...
        ## show overlay
        self.waterOverlay = Ogre.OverlayManager.getSingleton().getByName("Example/self.waterOverlay") 
        self.waterOverlay.show() 

        ## Let there be rain
        self.particleSystem = sceneManager.createself.particleSystem("rain",
            "Examples/Water/Rain") 
