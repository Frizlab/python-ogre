...
void prepareCircleMaterial()
	...
	for(int b=0 b<16;b++) {
		int x0 = b % 4  
		int y0 = b >> 2  
		Real radius = 4.0 + 1.4 * (float) b  
		for(int x=0 x<64;x++) {
			for(int y=0 y<64;y++) {
				Real dist = Math.Sqrt((x-32)*(x-32)+(y-32)*(y-32))  ## 0..ca.45
				dist = fabs(dist -radius -2) / 2.0  
				dist = dist * 255.0 
				if (dist>255):
					dist=255  
				int colour = 255-(int)dist  
				colour = (int)( ((Real)(15-b))/15.0 * (Real) colour ) 
	...
	void _prepareMesh()
		...
		if (first) { ## first Circle, create some static common data:
			...
			for(i=0 i<numVertices;i++) {
				posnormBufData[6*i+0]=((Real)(i%2)-0.5)*CIRCLE_SIZE  ## pos X
				posnormBufData[6*i+1]=0  ## pos Y
				posnormBufData[6*i+2]=((Real)(i/2)-0.5)*CIRCLE_SIZE  ## pos Z
		...
		## set mesh bounds
		AxisAlignedBox circleBounds(-CIRCLE_SIZE/2.0, 0, -CIRCLE_SIZE/2.0f,
...
public:
	...
	WaterCircle( String& name, Real x, Real y)
