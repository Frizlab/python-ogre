...
def AutoExclude( mb ):
    ...
    main_ns.calldefs( query, allow_empty=True ).exclude()
...
# # #         , messages.W1014 # unsupported '=' operator
    ...
    defined_symbols.append( 'VERSION_' + environment.betagui.version )  
    ...
    mb = module_builder.module_builder_t( [ xml_cached_fc ]
                                          ...
                                          , include_paths=environment.betagui.include_dirs
                                          , define_symbols=defined_symbols
                                          , indexing_suite_version=2
                                          , cflags=environment.betagui.cflags
                                           )
...
#     Set_Call_Policies ( mb.global_ns.namespace ('Ogre') )  ## need this as we are forcing singleton classes 
    ...
    mb.code_creator.user_defined_directories.append( environment.betagui.generated_dir )
    mb.code_creator.replace_included_headers( customization_data.header_files( environment.betagui.version ) )

    huge_classes = map( mb.class_, customization_data.huge_classes( environment.betagui.version ) )

    mb.split_module(environment.betagui.generated_dir, huge_classes,use_files_sum_repository=False)

    ## now we need to ensure a series of headers and additional source files are
    ## copied to the generated directory..
    additional_files= os.listdir(environment.Config.PATH_INCLUDE_betagui)
    for f in additional_files:
        if f.endswith('cpp') or f.endswith('.h'):
            sourcefile = os.path.join(environment.Config.PATH_INCLUDE_betagui, f)
            destfile = os.path.join(environment.betagui.generated_dir, f ) 
        
            if not common_utils.samefile( sourcefile ,destfile ):
                shutil.copy( sourcefile, environment.betagui.generated_dir )
