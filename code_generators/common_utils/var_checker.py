...
class var_checker:
    ...
    def __call__(self, declaration):
        log_info( "var_checker called with:"+ declaration)
        if not declaration.location.file_name :
            log_info( __file__+ ": location file name is empty"+declaration)
            return False 
        if self.file_name != declaration.location.file_name:            
            ...
            try:    
                ...
                log_info( __file__+ ": Unable to open "+ self.file_name+ "source is unknown"+declaration)
...
#         print declaration.location.line, len(self.source)
        try:
            ...
            log_info( __file__+ ":IndexError "+ self.source+ declaration.location.line)
            return False # Bug showed up with Caelum where decl location was bigger than source ??
        except TypeError:
            log_info( __file__+ ":TypeError "+ self.source+declaration.location.line)
            return False
