...
# Note that only Python 2.6 and later is supported
...
from . import var_checker as varchecker
from . import ogre_properties as ogre_properties
from . import PO_FuncTransform
from . import shared_ptr
...
class private_decls_t:
        ...
        for include_dir in include_dirs:    
            for fname in os.listdir( include_dir ):
                ...
                if not os.path.isfile( full_name ):
                    continue
                fobj = io.open( full_name, 'r', encoding='utf_8', errors='replace' )
                for index, line in enumerate( fobj ):
                    if '_OgrePrivate' in line:
                        if not fname in self.__private:
    ...
    def is_private( self, decl ):
        ...
        return file_name in self.__private and decl.location.line in self.__private[ file_name ]
    ...
    for unnamed_cls in classes:        
        ...
        for mvar in unnamed_cls.public_members:
            ...
            try:               
                ...
                log_warning ("**** Error in unnamed_classes", mvar)
            log_info ("Fixing Unnamed Class:", unnamed_cls, mvar, named_parent.name)
...
def remove_DuplicateProperties ( cls ):
    ...
    if duplicate:
        for k in unique.keys():  
            if len ( unique[k] ) > 1  :
                log_info ("DUPLICATE FOUND: ", cls, len ( unique[k] ))
                for p in unique[k]:
                    prefered = p
...
def Auto_Document ( mb, namespace=None ):
    ...
    for c in main_ns.calldefs( query, allow_empty=True ): #protected
        s = c.documentation
        if not s:
            s = ""
            ...
            if fullname not in ignore_funs and not fun.ignore:
                ...
                for arg in fun.arguments:
                    ...
                    if declarations.is_arithmetic (rawarg)\
                        ...
                        if declarations.is_pointer(arg.type):   #we convert any pointers to unsigned int's
                            ...
                            log_warning ("- possible code change.", fun,arg," not wrapped as const reference to base type invalid")
                        else:
                ...
                if trans:
                    ...
                    if fun.documentation or fun.transformations:   # it's already be tweaked:
                        log_warning ("AUTOFT ERROR: Duplicate Tranforms.", fun, fun.documentation)
                    ...
                    # if the class has a protected destruction AND the return value is const or a non arithmatic value then exclude it.
                        ...
                        log_warning ("AUTOFT ERROR Const: Parent has non public destructor and const return.", fun.parent.name, fun.return_type.decl_string, fun)
                        ...
                        log_warning ("AUTOFT ERROR Const: Parent has non public destructor and complex return value.", fun.parent.name, fun.return_type.decl_string, fun)
                        ...
                        if ctypes_conversion:   # only manage name changes if ctypes changing
                            ...
                            if len (s) > 1: 
                                ...
                                log_info ("Adjusting Alias as multiple overlapping functions:", new_alias)
                            
                        log_info ("AUTOFT OK: Tranformed ", fun.return_type.decl_string, fun, "(",new_alias,")")
...
#             else:
            log_info ("Tranformation applied to ", fun, desc, fun.virtuality)
            ...
            if declarations.is_pointer(arg.type): ## and "const" not in arg.type.decl_string:
                for i in pointee_types:
                    if arg.type.decl_string.startswith(i):
                        if Exclude: 
                            log_warning ("Excluding:", fun," due to pointer argument", arg.type.decl_string)
                            fun.exclude()
                            return
                        else:                        
#                             trans.append( ft.inout(arg_position) )
#                             desc = desc +"Argument: "+arg.name+ "( pos:" + str(arg_position) +") now an in/out. "
                            log_warning ("Function has pointer argument: ", fun, arg.type.decl_string)
            ...
            for i in pointee_types:
                if fun.return_type.decl_string.startswith ( i ) and not fun.documentation:
                    if not fun.name in known_names:
                        log_warning ("Func (", fun.name, ") returns ", i, ".Using ctype return_addressof functionality")
...
#                         fun.exclude()
    for fun in mb.member_operators( allow_empty = True ):
        if declarations.is_pointer (fun.return_type) and not fun.documentation:
            for i in pointee_types:
                if fun.return_type.decl_string.startswith ( i ) and not fun.documentation:
                    log_warning ("Excluding (operator):", fun)
                    fun.exclude()
    ...
    for fun in mb.free_functions( allow_empty = True ):
        if declarations.is_pointer (fun.return_type) and not fun.documentation:
            for i in pointee_types:
                if fun.return_type.decl_string.startswith ( i ) and not fun.documentation:
                    if not fun.name in known_names:
                        log_warning ("Excluding (free function):", fun, "as it returns (pointer)", i)
                        fun.exclude()
    ...
    #                 if not v.parent.noncopyable:    ## this test as we don't support classes with protected destructors
                    log_info ("CTYPE Implementation on ", v, v.access_type)
        ...
        if len (f.transformations) > 0:
            if f.parent.find_out_member_access_type( f ) == ACCESS_TYPES.PUBLIC:
                if f.virtuality == VIRTUALITY_TYPES.PURE_VIRTUAL:
                    log_warning ("Pure Virtual function with Transformations - they will not be applied:", f)
...
def Remove_Static_Consts ( mb ):
    ...
    for var in mb.vars():
        if type(var.type) == declarations.cpptypes.const_t:
            if checker( var ):
                log_info ("Excluding static const ", var)
                var.exclude()                         
                ...
                if v.is_read_only and v.exportable==True : # only if it's exposed..
                    if type (v.type) == declarations.cpptypes.const_t: # only care about const's
                        ...
                        if not known:
                            ...
                            log_info ("Excluding Problem Const", cls.name, v)
...
def copyTree ( sourcePath, destPath, recursive=False, extensions=['cpp','h','hxx','cxx','hpp'], collapse = False, files_in=None ):
    ...
    def copyFile ( fileName, sourcePath, destPath):
        ...
        if not samefile( sourceFile ,destFile ):
            ...
            log_info ("Updated ", destFile, "as it was missing or out of date")
