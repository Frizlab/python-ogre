...
caelum="xxx"  ##do a search and replace on caelum
...
def ManualInclude ( mb ):
    ...
    global_ns.namespace( 'Ogre' ).class_('RenderTargetViewportEvent').include(already_exposed=True)
    ...
    global_ns.namespace( 'Ogre' ).class_('GpuProgramParameters').include(already_exposed=True)
    global_ns.class_( '::Ogre::Image' ).include(already_exposed=True)
    global_ns.class_( '::Ogre::Degree' ).include(already_exposed=True)
    global_ns.class_( '::Ogre::Radian' ).include(already_exposed=True)
    global_ns.class_( '::Ogre::Root' ).include(already_exposed=True)
    global_ns.class_( '::Ogre::Exception' ).include(already_exposed=True)
    global_ns.class_( '::Ogre::TextureUnitState' ).include(already_exposed=True)
    #global_ns.class_( '::SharedPtr<Ogre::GpuProgramParameters>' ).include(already_exposed=True)
...
# # #         , messages.W1014 # unsupported '=' operator
    ...
    ##additional_files= os.listdir(environment.Config.PATH_INCLUDE_caelum)
    ##for f in additional_files:
    ##    if f.endswith('cpp') or f.endswith('.h'):
    ##        sourcefile = os.path.join(environment.Config.PATH_INCLUDE_caelum, f)
    ##        destfile = os.path.join(environment.caelum.generated_dir, f ) 
    ##    
    ##        if not common_utils.samefile( sourcefile ,destfile ):
    ##            shutil.copy( sourcefile, environment.caelum.generated_dir )
