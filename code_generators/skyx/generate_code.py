...
## STARTER TEMPLATE..
## replace skyx with lowercase project name
## set MAIN_NAMESPACE
...
def ManualExclude ( mb ):
    ...
    if MAIN_NAMESPACE:
        ...
        main_ns = global_ns    
    exclude = ['::SkyX::VClouds::FastFakeRandom::get',  # returns a address in a float array - needs different handling
                ]
    for e in exclude:
        try:
...
#           Warnings 1020 - 1031 are all about why Py++ generates wrapper for class X
    ...
    xml_cached_fc = parser.create_cached_source_fc(
                        os.path.join( environment.skyx.root_dir, "python_skyx.h" )
                        , environment.skyx.cache_file )

    defined_symbols = environment.defined_symbols 
    defined_symbols.append( 'VERSION_' + environment.skyx.version,  )  
    ...
    mb = module_builder.module_builder_t( [ xml_cached_fc ]
                                          ...
                                          , include_paths=environment.skyx.include_dirs
                                          , define_symbols=defined_symbols
                                          , indexing_suite_version=2
                                          , cflags=environment.skyx.cflags
                                           )
    ...
    common_utils.addDetailVersion ( mb, environment, environment.skyx )
    ...
    mb.build_code_creator (module_name='_skyx_' , doc_extractor= extractor )
    
    for inc in environment.skyx.include_dirs:
        mb.code_creator.user_defined_directories.append(inc )
    mb.code_creator.user_defined_directories.append( environment.skyx.generated_dir )
    mb.code_creator.replace_included_headers( customization_data.header_files( environment.skyx.version ) )

    huge_classes = map( mb.class_, customization_data.huge_classes( environment.skyx.version ) )

    mb.split_module(environment.skyx.generated_dir, huge_classes, use_files_sum_repository=False)

    ## now we need to ensure a series of headers and additional source files are
    ## copied to the generated directory..
    
#     common_utils.copyTree ( sourcePath = environment.Config.PATH_INCLUDE_skyx, 
#                             destPath = environment.skyx.generated_dir, 
#                             recursive=False )
