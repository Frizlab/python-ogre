...
## STARTER TEMPLATE..
## replace doctester with lowercase project name
## set MAIN_NAMESPACE
...
#           Warnings 1020 - 1031 are all about why Py++ generates wrapper for class X
    ...
    xml_cached_fc = parser.create_cached_source_fc(
                        os.path.join( environment.doctester.root_dir, "python_doctester.h" )
                        , environment.doctester.cache_file )

    defined_symbols = [ ]
    defined_symbols.append( 'VERSION_' + environment.doctester.version )  
    ...
    mb = module_builder.module_builder_t( [ xml_cached_fc ]
                                          ...
                                          , include_paths=environment.doctester.include_dirs
                                          , define_symbols=defined_symbols
                                          , indexing_suite_version=2
                                          , cflags=environment.doctester.cflags
                                           )
    ...
    common_utils.addDetailVersion ( mb, environment, environment.doctester )
    ...
    mb.build_code_creator (module_name='_doctester_' , doc_extractor= extractor )
    
#     for inc in environment.doctester.include_dirs:
#         mb.code_creator.user_defined_directories.append(inc )
#     mb.code_creator.user_defined_directories.append( environment.doctester.generated_dir )
#     mb.code_creator.replace_included_headers( customization_data.header_files( environment.doctester.version ) )

    huge_classes = map( mb.class_, customization_data.huge_classes( environment.doctester.version ) )

    mb.split_module(environment.doctester.generated_dir, huge_classes, use_files_sum_repository=False)

    ## now we need to ensure a series of headers and additional source files are
    ## copied to the generated directory..
    
#     common_utils.copyTree ( sourcePath = environment.Config.PATH_INCLUDE_doctester, 
#                             destPath = environment.doctester.generated_dir, 
#                             recursive=False )
