...
##    sys.exit()
    ...
    # have an issue with large enums not being handled correctly by GCC-XML and Python 3
    # seems gcc-xml uses a singned long instead of unsigned so large values become negative
    # and then handled in boost makes them
    exclude_list = ['ProfileGroupMask' , 'TextureMipmap']
    for var in main_ns.enums ():
      if var.name in exclude_list:
         var.exclude()
         log_exclude( var.name, extra="ENUM with value too large to handle")
         continue
      for name, v in var.values:
         if v >= 0x40000000:
            log_exclude( var.name, extra="ENUM with value too large to handle:" + name)
            var.exclude()
            continue
