...
## STARTER TEMPLATE..
## replace ofusion with lowercase project name
## set MAIN_NAMESPACE
...
def ManualInclude ( mb ):
    ...
    if MAIN_NAMESPACE:
        ...
        main_ns = global_ns    
    main_ns.class_('OSMScene').include()   
    main_ns.class_('OSMSceneCallbacks').include()   
...
# #          )
    ...
    mb = module_builder.module_builder_t( [ xml_cached_fc ]
                                          ...
                                          , include_paths=environment.ofusion.include_dirs
                                          , define_symbols=defined_symbols
                                          , indexing_suite_version=2
                                          , cflags=environment.ofusion.cflags
                                           )
    ...
    mb.build_code_creator (module_name='_ofusion_' , doc_extractor= extractor )
    
    for inc in environment.ofusion.include_dirs:
        mb.code_creator.user_defined_directories.append(inc )
    mb.code_creator.user_defined_directories.append( environment.ofusion.generated_dir )
    mb.code_creator.replace_included_headers( customization_data.header_files( environment.ofusion.version ) )

    huge_classes = map( mb.class_, customization_data.huge_classes( environment.ofusion.version ) )

    mb.split_module(environment.ofusion.generated_dir, huge_classes, use_files_sum_repository=False)

    ## now we need to ensure a series of headers and additional source files are
    ## copied to the generated directory..
    
    common_utils.copyTree ( sourcePath = environment.Config.PATH_ofusion, 
                            destPath = environment.ofusion.generated_dir, 
                            recursive=False )
