...
button *  button::hide(bool fade, unsigned short fadeDurationMS)
{
	if(!getVisibility()) 
		return this;
	if(fadingOut)	//adlready busy hiding.
		return this;
...
{
	if(getVisibility()) //already showing
		return this;
...
button *  button::setPosition(buttonPosition &p)
{
	...
	if(position.usingRelative)
	{
		...
		int center = (winWidth/2)-(width/2) + position.data.rel.x;
			if (parent)	center = (parent->panel->getWidth()/2)-(width/2) + position.data.rel.x; //if this is a child, this should be relative to parent.
		int right = winWidth - width - position.data.rel.x;
			if (parent)	right = parent->panel->getWidth() - width - position.data.rel.x; //if this is a child, this should be relative to parent.

		int top = 0 + position.data.rel.y;
		int middle = (winHeight/2)-(height/2) + position.data.rel.y;
			if (parent)	middle = (parent->panel->getHeight()/2)-(height/2) + position.data.rel.y; //if this is a child, this should be relative to parent.
		int bottom = winHeight - height - position.data.rel.y;
			if (parent)	bottom = parent->panel->getHeight() - height - position.data.rel.y; //if this is a child, this should be relative to parent.
	...
	else	//using absolute position (even local positions fall back here.since this function calls itself as absolute when it is called as relative.)
	{
		...
		{	//we are a child so we need to get the offset we are inheriting from parents so we can set to our actual world position by using the local position with an offset
			int currentLocalLeft = panel->getLeft();
			int currentLocalTop = panel->getTop();
			...
			{
				if (!checkTargetPositionX(position.data.abs.left))  //this deals in absolutes only
					targetLocalLeft = currentLocalLeft;  //dont move in this axis
				if (!checkTargetPositionY( position.data.abs.top))
					targetLocalTop = currentLocalTop;  //dont move in this axis
			}	
			panel->setPosition(targetLocalLeft, targetLocalTop );
		}
		else
		{
			if (useLimits) //check if limits are even imposed on this button
			{
				if (!checkTargetPositionX(position.data.abs.left))  //this deals in absolutes only
					position.data.abs.left = panel->getLeft();  //dont move in this axis
				if (!checkTargetPositionY( position.data.abs.top))
					position.data.abs.top = panel->getTop();  //dont move in this axis
			}	
...
bool button::checkTargetPositionX(int x)
{
		int leftLimit = x1Limit;
		int rightLimit = x2Limit;

		if (overlay == NULL) //are we a child?
		{
			buttonPosition parentPos = parent->getAbsolutePosition();  //offset by the parent
			leftLimit = parentPos.data.abs.left + x1Limit;
			rightLimit = parentPos.data.abs.left + x2Limit;
		}
...
bool button::checkTargetPositionY( int y)
{
		int topLimit = y1Limit;
		int bottomLimit = y2Limit;

		if (overlay == NULL) //are we a child?
		{
			buttonPosition parentPos = parent->getAbsolutePosition();  //offset by the parent
			topLimit = parentPos.data.abs.top + y1Limit;
			bottomLimit = parentPos.data.abs.top + y2Limit;
		}
...
bool button::hitTest(int x, int y)
{
	...
	int right = pos.data.abs.left + panel->getWidth(); 
	int bottom = pos.data.abs.top + panel->getHeight();
...
std::string * textInputArea::getValue(void)
{
	return &textValue;
}
...
buttonMesh::buttonMesh( const Ogre::String name, const Ogre::String meshFile, buttonManager * buttonMgr, button* parentButton, short unsigned int zOrder,  short positionX, short positionY,  short width, short height) :
		...
		momentumY(0),
		friction(.035),
		turnSpeedModifier(.025) //less makes it turn slower
{
	...
	Vector3 bbox = e->getBoundingBox().getSize();
...
{
	Quaternion quat = Quaternion(Degree(x),Vector3::UNIT_X);
    Quaternion quat1 = Quaternion(Degree(y),Vector3::UNIT_Y);
	Quaternion quat2 = Quaternion(Degree(z),Vector3::UNIT_Z);
...
void buttonMesh::update(void)  
{
	buttonPosition p = parent->getAbsolutePosition();
			setPosition(p.data.abs.left + relX, p.data.abs.top + relY); //we can't truly parent buttonMeshes to buttons.  so I semi-hardcore update their positions each frame here
		...
		if(rotatableX)
		{
			if (momentumX)
			{
				baseNode->yaw(Ogre::Radian(-momentumX * turnSpeedModifier), Ogre::Node::TS_WORLD);
					...
					momentumX = momentumX  - friction * buttonMgr->timeDiff;
				if (momentumX <= 0)
					momentumX = 0;
			}
		}
		if(rotatableY)
		{
			if (momentumY)
			{
				baseNode->pitch(Ogre::Radian(-momentumY * turnSpeedModifier), Ogre::Node::TS_WORLD);	
					...
					momentumY = momentumY - friction * buttonMgr->timeDiff;
				if (momentumY <= 0)
					momentumY = 0;
			}
		}
...
void buttonMesh::injectMouseMove(short &x, short &y)
{
	if(mouseOn)
	{
		if((rotatableX)||(rotatableY))
		{
			momentumX = lastPos.x - x;
			momentumY = lastPos.y - y;
			lastPos.x = x;
			lastPos.y = y;
		}
	}
...
{	
	...
	if (arg.key == KC_LEFT)  //caps lock
	{
		sceneMgr->getSceneNode("camNode")->yaw(Ogre::Radian(.2));
	}
		if (arg.key == KC_RIGHT)  //caps lock
	{
		sceneMgr->getSceneNode("camNode")->yaw(Ogre::Radian(-.2));
	}
...
bool buttonManager::injectMouseMove(buttonPosition &position)
{
	if(position.usingRelative)
	{
		int winWidth = screenResX;
		int winHeight = screenResY;
		float width = cursorButton->panel->getWidth();
		float height = cursorButton->panel->getHeight();
		...
		{
		case LEFT:
			injectMouseMove(left, middle);
			break;
		case TOP_LEFT:
			injectMouseMove(left, top);
			break;
		case TOP_CENTER:
			injectMouseMove(center, top);
			break;
		case TOP_RIGHT:
			injectMouseMove(right, top);
			break;
		case RIGHT:
			injectMouseMove(right, middle);
			break;
		case BOTTOM_RIGHT:
			injectMouseMove(right, bottom);
			break;
		case BOTTOM_CENTER:
			injectMouseMove(center, bottom);
			break;
		case BOTTOM_LEFT:
			injectMouseMove(left, bottom);
			break;
		case CENTER:
			injectMouseMove(center, middle);
			break;
		default:
			injectMouseMove(position.data.rel.x, position.data.rel.y);
			break;
		}
	}
	else	//using absolute position (even local positions fall back here.since this function calls itself as absolute when it is called as relative.)
	{
			injectMouseMove(position.data.abs.left, position.data.abs.top);
	}
	return true;
}
...
bool buttonManager::injectMouseMove(int xPos, int yPos)
{
	...
	{	
		...
		{
			int width = grabbedButton->panel->getWidth()/2;  
			int height = grabbedButton->panel->getHeight()/2;
			grabbedButton->setPosition(buttonPosition(xPos - width, yPos - height));
		}
		else	//do a relative drag.
		{
			buttonPosition p = grabbedButton->getAbsolutePosition();
			grabbedButton->setPosition(buttonPosition(p.data.abs.left +  xPos - mouseX, p.data.abs.top + yPos - mouseY));  //current pos + old mousePos - new mousePos
		}
	}
