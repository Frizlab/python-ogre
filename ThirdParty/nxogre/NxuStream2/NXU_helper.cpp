...
static void		  saveForceFields(NXU::NxuPhysicsCollection *c,NxScene *scene)
{
	...
	if (fieldCount)
	{
		...
		for	(NxU32 i = 0;	i	<	fieldCount; ++i)
		{
			...
			//NxForceField *f	=	scene->getNextEffector();
			//effectors.push_back(e);
		}
		//for (NxU32 i=0; i<effectorCount; ++i)
		//{
...
bool addToCurrentSceneInstance(NxuPhysicsCollection &c,const char *sceneId,const char *sceneName,const NxMat34 &rootNode,bool ignorePlane,const char *userProperties)
{
	bool ret = false;

  NxuPhysicsExport exporter(&c);

	ret = exporter.addToCurrentSceneInstance(sceneId,sceneName,rootNode,ignorePlane,userProperties);
