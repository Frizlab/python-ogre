								...
								const int tileX, const int tileZ)
{
	...
	PLSM2_ASSERT (MaxHeight >= (Real)0.0f);
...
{
	PLSM2_ASSERT (mLoaded && mRenderable);
...
{
	PLSM2_ASSERT (mInit && !mLoaded && mRenderable == 0);
	...
	else
	{
			...
			(ray.z < bottomBorder))
		{
			...
			if (ray.y <= localMax) // until under the max possible for this page/tile
			{
				...
				// otherwise we'll hit PLSM2_ASSERTs.
...
{
	PLSM2_ASSERT (mInit && mLoaded && mRenderable);
