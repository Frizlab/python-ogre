...
namespace Ogre
{
    ...
    {        
		PLSM2_ASSERT (!mData2DTypeMap.empty() && mData2DTypeMap[mData2DType]);
    ...
    const Real PagingLandScapeData2DManager::getInterpolatedWorldHeight (const Real x, const Real z, Real *getSlopeAt)
    {
      ...
      Real heightValue;
			...
			localTileZ >= tileSizeMinuscurrentLodStep))
		{
			...
			PLSM2_ASSERT (north != south || (south == false && north == false));
			...
			PLSM2_ASSERT (east != west || (east == false && west == false));
