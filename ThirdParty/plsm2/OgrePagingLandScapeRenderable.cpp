...
#define TEXTURE_BINDING		1
...

    {
        ...
        if (mParent->getOptions ()->lodMorph)
        {
            ...
            PLSM2_ASSERT (mParent->getOptions ()->maxRenderLevel > 1);
           ...
           //decl->addElement(DELTA_BINDING, 0, VET_SHORT2, VES_TEXTURE_COORDINATES, 1);
            ...
            {
               PLSM2_ASSERT (mDeltaBuffers[k].isNull());
   ...
   void PagingLandScapeRenderable::visitRenderables(Renderable::Visitor* visitor, bool debugRenderables)
   {
	...
	{
		PLSM2_ASSERT (mQueued || mNeedReload);
	    PLSM2_ASSERT (mInfo);
		//if (!mInUse)
		//{
...
//                +  " | " + StringConverter::toString(mRect.top) + " - " + StringConverter::toString(mRect.bottom));
    ...
    {
        PLSM2_ASSERT (mIsLoaded && mInfo);
		PLSM2_ASSERT (!mQueued);
        if (mNeighbors[SOUTH])
                ...
                if (mParent->getOptions ()->lodMorph)
                {
                    ...
                    else
                    {
                        ...
                        if (range)
                        {
                            ...
                            mCustomGpuParameters.w = mLODMorphFactor;
                            PLSM2_ASSERT (mLODMorphFactor <= 1.0f);
                        }
                        else
                        {
                            // Identical ranges
    ...
    {   
       ...
       mLightListDirty = true;
       PLSM2_ASSERT (mInUse &&  mParentNode && mIsLoaded && mVisible);
...
// should change if we're just refilling a part
        ...
        PLSM2_ASSERT (IntervallModifiedZ * tilesize > 0);

        // until we found a way to upload part of the data.
        PLSM2_ASSERT (IntervallModifiedZ == tilesize);
...
// should change if we're just refilling a part to whole tile (only if using discard, tough)
                ...
                {
                    // Save height    
                    PLSM2_ASSERT (k < static_cast<int> (size));
...
// even with offset that would make us in "middle of a step"
                    ...
                    for (int z = 0; z <= zubound; z++)
                    {
                        ...
                        const int zPageSize = z * pageSize;
                        {
                            ...
                            for (int x = 0; x <= xubound; x++)
                            {
                                ...
                                PLSM2_ASSERT  ((fulldetailx + K_heightFieldPos + zPageSize) < (pageSize*pageSize));
                                ...
                                {   
                                    ...
                                    if (delta != 0.0f)
                                    {
                                        ...
                                        {         
                                            ...
                                            PLSM2_ASSERT ((tileposx + (tileposy * tilesize))*blendWeights < size);
    ...
    {        
        PLSM2_ASSERT (x < mParent->getOptions ()->TileSize && z < mParent->getOptions ()->TileSize);
        if (mIsRectModified)
        {
            if (mRect.left > x)
                mRect.left = x;
