...
namespace QuickGUI
{
	...
	void Widget::_notifyDimensionsChanged()
	{
		...
		// Update borders to window position
	...
	void Widget::_notifyZOrder(Ogre::ushort windowZOrder, Ogre::ushort offsetZOrder)
	{
		mOffsetZOrder = offsetZOrder;
		// This function is virtual, and widgets with child widgets will update them via this call.
		_notifyZOrder(windowZOrder);
	}

	void Widget::_notifyZOrder(Ogre::ushort windowZOrder)
	{
		mWindowZOrder = windowZOrder;
	}
	...
	Ogre::String Widget::getReferenceName()
	{
	...
	{
		if( (mWidgetType == Widget::QGUI_TYPE_WINDOW) || (mWidgetType == Widget::QGUI_TYPE_PLAINWINDOW) )
			return dynamic_cast<Window*>(this);

		Widget* w = mParentWidget;
		while( w != NULL )
		{
			if( (w->getWidgetType() == Widget::QGUI_TYPE_WINDOW) || (w->getWidgetType() == Widget::QGUI_TYPE_PLAINWINDOW) ) break;
	...
	Ogre::ushort Widget::getDerivedZOrder()
	{
		return (mWindowZOrder + mOffsetZOrder);
	}
	...
	void Widget::setReferenceName(const Ogre::String& ReferenceName)
	{
