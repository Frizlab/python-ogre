...
namespace QuickGUI
{
		...
		mCursorOffset(-3)
	{
		...
		Ogre::String tcMaterial = QuickGUI::GUIManager::getSingleton().getDefaultSkin() + ".textcursor";
		mTextCursor = new TextCursor(mInstanceName+"_TextCursor",Ogre::Vector4(0,0,0,1),tcMaterial,mOverlayContainer,this);
		mTextCursor->hide();
	}
	...
	void Text::_notifyDimensionsChanged()
	{
		...
		//mTextCursor->_notifyDimensionsChanged();
	}
