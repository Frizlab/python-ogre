...
namespace QuickGUI
{
	...
	TreeViewCheckBoxNodeDesc::TreeViewCheckBoxNodeDesc(const Ogre::String& id) :
		TreeViewNodeDesc(id)
	{
		resetToDefault();
	}
	...
	{		
		...
		setWidth((tvcbnd->widget_dimensions.size.height * 3) + tvcbnd->textDesc.getTextWidth() + 5);

		mDesc = dynamic_cast<TreeViewCheckBoxNodeDesc*>(mWidgetDesc);

		CheckBoxDesc* cbd = FactoryManager::getSingleton().getDescFactory()->getInstance<CheckBoxDesc>("DefaultCheckBoxDesc");
		cbd->resetToDefault();
		...
		mCheckBox = dynamic_cast<CheckBox*>(Widget::create("CheckBox",cbd));
	...
	{
		if(mTreeViewCheckBoxNodeEventHandlers[e].empty())
			return false;
