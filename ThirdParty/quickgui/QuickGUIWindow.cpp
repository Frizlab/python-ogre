...
namespace QuickGUI
{
	Window::Window(const Ogre::String& name, const Ogre::Vector4& dimensions, GuiMetricsMode positionMode, GuiMetricsMode sizeMode, const Ogre::String& material, Widget* parentWidget) :
		Panel(name,dimensions,positionMode,sizeMode,material,NULL,parentWidget),
		mTitleBar(0),
		mTitleBarHidden(0)
	{
		...
		Ogre::Vector4 defaultTitleBarDimensions = Ogre::Vector4(0,0,1,0.05 / getSize(QGUI_GMM_ABSOLUTE).y);
		mTitleBar = new TitleBar(mInstanceName+".Titlebar",defaultTitleBarDimensions,QGUI_GMM_RELATIVE,QGUI_GMM_RELATIVE,mWidgetMaterial+".titlebar",mTitleBarContainer,this);
	...
	void Window::show()
	{
		...
		if( mTitleBarHidden ) hideTitlebar(); 
	}
