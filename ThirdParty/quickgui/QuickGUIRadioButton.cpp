...
namespace QuickGUI
{
	...
	RadioButtonDesc::RadioButtonDesc(const Ogre::String& id) :
		WidgetDesc(id)
	{
		resetToDefault();
	}
	...
	{
		if(mRadioButtonEventHandlers[e].empty())
			return false;
	...
	void RadioButton::onMouseEnter(const EventArgs& args)
	{
		if(mDesc->radiobutton_selected)
			mSkinElementName = SELECTED_OVER;
		else
			mSkinElementName = UNSELECTED_OVER;
	...
	void RadioButton::onMouseLeftButtonDown(const EventArgs& args)
	{
		...
		{
			if(mDesc->radiobutton_selected)
				mSkinElementName = SELECTED_DOWN;
			else
				mSkinElementName = UNSELECTED_DOWN;
		...
		if((mParentWidget != NULL) && (mParentWidget->isContainerWidget()))
		{
			...
			for(std::vector<Widget*>::iterator it = children.begin(); it != children.end(); ++it)
			{
				if((*it)->getClass() == "RadioButton")
				{
					...
					rb->mSkinElementName = UNSELECTED;
				}
			}
		}
