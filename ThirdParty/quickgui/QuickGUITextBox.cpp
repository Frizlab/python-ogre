...
namespace QuickGUI
{
		...
		mInputMode(0)
	{
		...
		mTextWidget = new Text(mInstanceName+"_Text",textDimensions,mChildrenContainer,this);
	...
	void TextBox::_notifyZOrder(Ogre::ushort windowZOrder, Ogre::ushort offsetZOrder)
	{
		Widget::_notifyZOrder(windowZOrder,offsetZOrder);
	}

	void TextBox::_notifyZOrder(Ogre::ushort windowZOrder)
	{
	...
	Widget* TextBox::getTargetWidget(const Ogre::Vector2& p)
	{
		if( !mVisible ) return NULL;
		else if( mTextWidget->getTargetWidget(p) ) return mTextWidget->getTargetWidget(p);
	...
	{
		mOverlayElement->setTiling(horizontalRepetitions,verticalRepetitions,layer);
	}
