...
#endif
#if NX_SDK_VERSION_NUMBER >= 280
    case NX_SYMMETRIC_FRICTION: ret = "NX_SYMMETRIC_FRICTION"; break;
#endif
...
#endif
#if NX_SDK_VERSION_NUMBER >= 280
  else if ( _stricmp(str,"NX_SYMMETRIC_FRICTION") == 0 ) { v = NX_SYMMETRIC_FRICTION; ret = true; }
#endif
...
{
  bool ret = false;
       if ( _stricmp(str,"NX_CF_SLEEP_NOTIFICATION") == 0 ) { v = NX_CF_SLEEP_NOTIFICATION; ret = true; }
#if NX_SDK_VERSION_NUMBER >= 271
...
bool         StringToEnum(const char *str,NxForceFieldCoordinates &v)
{
  ...
  if ( str )
  {
       if ( _stricmp(str,"NX_FFC_CARTESIAN") == 0 ) { v = NX_FFC_CARTESIAN; ret = true; }
...
void NxConvexMeshDesc::store(SchemaStream &stream,const char *parent)
{
  ...
  if ( gSaveDefaults || def.mUserProperties != mUserProperties )
    stream.store(mUserProperties,"mUserProperties",false);    // {NXOGRE}
  if ( gSaveCooked )
  {
...
void NxSceneDesc::store(SchemaStream &stream,const char *parent)
{
  ...
  if ( gSaveDefaults || def.mId != mId )
    stream.store(mId,"mId",true);
  if ( gSaveDefaults || def.mUserProperties != mUserProperties )
    stream.store(mUserProperties,"mUserProperties",false);			// {NXOGRE} - Properties not attributes
...
void NxSceneDesc::load(SchemaStream &stream,const char *parent)
{
  ...
  {
    stream.load(mId,"mId",true);
    stream.load(mUserProperties,"mUserProperties",false);			// Properties aren't attributes
